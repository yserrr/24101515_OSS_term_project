#ifndef MYPROJECT_MNIST_COMMON_HPP
#define MYPROJECT_MNIST_COMMON_HPP
#include <algorithm>
#include <cstdint>
#include <random>
#include <string>
...
struct mnist_model
{
  ...
  struct MmlTensor* images = nullptr;
  struct MmlTensor* logits = nullptr;

  struct MmlTensor* fc1_weight = nullptr;
  struct MmlTensor* fc1_bias = nullptr;
  struct MmlTensor* fc2_weight = nullptr;
  struct MmlTensor* fc2_bias = nullptr;
    ...
    : nbatch_logical(nbatch_logical), nbatch_physical(nbatch_physical)
  {
    ...
    if (backends.size() >= 2)
    {
      fprintf(
        stderr,
        "%s: unsupported operations will be executed on the following fallback backends (in order of priority):\n",
        __func__);
      for (size_t i = 1; i < backends.size(); ++i)
      {
        fprintf(stderr, "%s:  - %s (%s)\n", __func__, ggml_backend_name(backends[i]),
...
#endif //MYPROJECT_MNIST_COMMON_HPP
