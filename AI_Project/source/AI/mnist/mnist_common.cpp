...
bool mnist_image_load(const std::string& fname, ggml_opt_dataset_t dataset)
{
  ...
  struct MmlTensor* images = ggml_opt_dataset_data(dataset);
  float* buf = ggml_get_data_f32(images);

  MML_ASSERT(images->ne[0] == MNIST_NINPUT);
...
bool mnist_label_load(const std::string& fname, ggml_opt_dataset_t dataset)
{
  ...
  uint8_t label;
  struct MmlTensor* labels = ggml_opt_dataset_labels(dataset);
  float* buf = ggml_get_data_f32(labels);

  MML_ASSERT(labels->ne[0] == MNIST_NCLASSES);
...
bool load_from_gguf(const char* fname, struct ggml_context* ctx_ggml, struct gguf_context* ctx_gguf)
{
  ...
  for (int i = 0; i < n_tensors; i++)
  {
    ...
    struct MmlTensor* tensor = ggml_get_tensor(ctx_ggml, name);
    if (!tensor)
    {
      continue;
    }
...
mnist_model mnist_model_init_from_file(const std::string& fname, const std::string& backend, const int nbatch_logical,
                                       const int nbatch_physical)
{
  ...
  if (model.arch == "mnist-fc")
  {
    ...
    MML_ASSERT(model.fc1_weight->ne[0] == MNIST_NINPUT);
    MML_ASSERT(model.fc1_weight->ne[1] == MNIST_NHIDDEN);
    MML_ASSERT(model.fc1_weight->ne[2] == 1);
    MML_ASSERT(model.fc1_weight->ne[3] == 1);

    model.fc1_bias = ggml_get_tensor(model.ctx_gguf, "fc1.bias");
    MML_ASSERT(model.fc1_bias->ne[0] == MNIST_NHIDDEN);
    MML_ASSERT(model.fc1_bias->ne[1] == 1);
    MML_ASSERT(model.fc1_bias->ne[2] == 1);
    MML_ASSERT(model.fc1_bias->ne[3] == 1);

    model.fc2_weight = ggml_get_tensor(model.ctx_gguf, "fc2.weight");
    MML_ASSERT(model.fc2_weight->ne[0] == MNIST_NHIDDEN);
    MML_ASSERT(model.fc2_weight->ne[1] == MNIST_NCLASSES);
    MML_ASSERT(model.fc2_weight->ne[2] == 1);
    MML_ASSERT(model.fc2_weight->ne[3] == 1);

    model.fc2_bias = ggml_get_tensor(model.ctx_gguf, "fc2.bias");
    MML_ASSERT(model.fc2_bias->ne[0] == MNIST_NCLASSES);
    MML_ASSERT(model.fc2_bias->ne[1] == 1);
    MML_ASSERT(model.fc2_bias->ne[2] == 1);
    MML_ASSERT(model.fc2_bias->ne[3] == 1);
  }
  else if (model.arch == "mnist-cnn")
  {
    model.conv1_kernel = ggml_get_tensor(model.ctx_gguf, "conv1.kernel");
    MML_ASSERT(model.conv1_kernel->type == GGML_TYPE_F32);
    MML_ASSERT(model.conv1_kernel->ne[0] == 3);
    MML_ASSERT(model.conv1_kernel->ne[1] == 3);
    MML_ASSERT(model.conv1_kernel->ne[2] == 1);
    MML_ASSERT(model.conv1_kernel->ne[3] == MNIST_CNN_NCB);

    model.conv1_bias = ggml_get_tensor(model.ctx_gguf, "conv1.bias");
    MML_ASSERT(model.conv1_bias->type == GGML_TYPE_F32);
    MML_ASSERT(model.conv1_bias->ne[0] == 1);
    MML_ASSERT(model.conv1_bias->ne[1] == 1);
    MML_ASSERT(model.conv1_bias->ne[2] == MNIST_CNN_NCB);
    MML_ASSERT(model.conv1_bias->ne[3] == 1);

    model.conv2_kernel = ggml_get_tensor(model.ctx_gguf, "conv2.kernel");
    MML_ASSERT(model.conv2_kernel->type == GGML_TYPE_F32);
    MML_ASSERT(model.conv2_kernel->ne[0] == 3);
    MML_ASSERT(model.conv2_kernel->ne[1] == 3);
    MML_ASSERT(model.conv2_kernel->ne[2] == MNIST_CNN_NCB);
    MML_ASSERT(model.conv2_kernel->ne[3] == MNIST_CNN_NCB * 2);

    model.conv2_bias = ggml_get_tensor(model.ctx_gguf, "conv2.bias");
    MML_ASSERT(model.conv2_bias->type == GGML_TYPE_F32);
    MML_ASSERT(model.conv2_bias->ne[0] == 1);
    MML_ASSERT(model.conv2_bias->ne[1] == 1);
    MML_ASSERT(model.conv2_bias->ne[2] == MNIST_CNN_NCB * 2);
    MML_ASSERT(model.conv2_bias->ne[3] == 1);

    model.dense_weight = ggml_get_tensor(model.ctx_gguf, "dense.weight");
    MML_ASSERT(model.dense_weight->type == GGML_TYPE_F32);
    MML_ASSERT(model.dense_weight->ne[0] == (MNIST_HW / 4) * (MNIST_HW / 4) * (MNIST_CNN_NCB * 2));
    MML_ASSERT(model.dense_weight->ne[1] == MNIST_NCLASSES);
    MML_ASSERT(model.dense_weight->ne[2] == 1);
    MML_ASSERT(model.dense_weight->ne[3] == 1);

    model.dense_bias = ggml_get_tensor(model.ctx_gguf, "dense.bias");
    MML_ASSERT(model.dense_bias->type == GGML_TYPE_F32);
    MML_ASSERT(model.dense_bias->ne[0] == MNIST_NCLASSES);
    MML_ASSERT(model.dense_bias->ne[1] == 1);
    MML_ASSERT(model.dense_bias->ne[2] == 1);
    MML_ASSERT(model.dense_bias->ne[3] == 1);
  }
  else
  {
...
mnist_model mnist_model_init_random(const std::string& arch, const std::string& backend, const int nbatch_logical,
                                    const int nbatch_physical)
{
  mnist_model model(backend, nbatch_logical, nbatch_physical);
  model.arch = arch;
  ...
  std::vector<MmlTensor*> init_tensors;
  ...
  else if (model.arch == "mnist-cnn")
  {
    ...
    model.dense_weight = ggml_new_tensor_2d(model.ctx_static, GGML_TYPE_F32,
  ...
  for (MmlTensor* t : init_tensors)
  {
    MML_ASSERT(t->type == GGML_TYPE_F32);
...
void mnist_model_build(mnist_model& model)
{
  if (model.arch == "mnist-fc")
  {
    ...
    MmlTensor* fc1 = ggml_relu(model.ctx_compute, ggml_add(model.ctx_compute,
                                                             ggml_mul_mat(
                                                               model.ctx_compute, model.fc1_weight, model.images),
                                                             model.fc1_bias));
  ...
  else if (model.arch == "mnist-cnn")
  {
    ...
    struct MmlTensor* images_2D = ggml_reshape_4d(model.ctx_compute, model.images, MNIST_HW, MNIST_HW, 1,
                                                    model.images->ne[1]);

    struct MmlTensor* conv1_out = ggml_relu(model.ctx_compute, ggml_add(model.ctx_compute,
                                                                          ggml_conv_2d(
                                                                            model.ctx_compute, model.conv1_kernel,
                                                                            images_2D, 1, 1, 1, 1, 1, 1),
                                                                          model.conv1_bias));
    MML_ASSERT(conv1_out->ne[0] == MNIST_HW);
    MML_ASSERT(conv1_out->ne[1] == MNIST_HW);
    MML_ASSERT(conv1_out->ne[2] == MNIST_CNN_NCB);
    MML_ASSERT(conv1_out->ne[3] == model.nbatch_physical);

    struct MmlTensor* conv2_in = ggml_pool_2d(model.ctx_compute, conv1_out, GGML_OP_POOL_MAX, 2, 2, 2, 2, 0, 0);
    MML_ASSERT(conv2_in->ne[0] == MNIST_HW / 2);
    MML_ASSERT(conv2_in->ne[1] == MNIST_HW / 2);
    MML_ASSERT(conv2_in->ne[2] == MNIST_CNN_NCB);
    MML_ASSERT(conv2_in->ne[3] == model.nbatch_physical);

    struct MmlTensor* conv2_out = ggml_relu(model.ctx_compute, ggml_add(model.ctx_compute,
                                                                          ggml_conv_2d(
                                                                            model.ctx_compute, model.conv2_kernel,
                                                                            conv2_in, 1, 1, 1, 1, 1, 1),
                                                                          model.conv2_bias));
    MML_ASSERT(conv2_out->ne[0] == MNIST_HW / 2);
    MML_ASSERT(conv2_out->ne[1] == MNIST_HW / 2);
    MML_ASSERT(conv2_out->ne[2] == MNIST_CNN_NCB * 2);
    MML_ASSERT(conv2_out->ne[3] == model.nbatch_physical);

    struct MmlTensor* dense_in = ggml_pool_2d(model.ctx_compute, conv2_out, GGML_OP_POOL_MAX, 2, 2, 2, 2, 0, 0);
    MML_ASSERT(dense_in->ne[0] == MNIST_HW / 4);
    MML_ASSERT(dense_in->ne[1] == MNIST_HW / 4);
    MML_ASSERT(dense_in->ne[2] == MNIST_CNN_NCB * 2);
    MML_ASSERT(dense_in->ne[3] == model.nbatch_physical);
    ...
    MML_ASSERT(dense_in->ne[0] == (MNIST_HW / 4) * (MNIST_HW / 4) * (MNIST_CNN_NCB * 2));
    MML_ASSERT(dense_in->ne[1] == model.nbatch_physical);
    MML_ASSERT(dense_in->ne[2] == 1);
    MML_ASSERT(dense_in->ne[3] == 1);

    model.logits = ggml_add(model.ctx_compute, ggml_mul_mat(model.ctx_compute, model.dense_weight, dense_in),
                            model.dense_bias);
  }
  else
  {
  ...
  MML_ASSERT(model.logits->type == GGML_TYPE_F32);
  MML_ASSERT(model.logits->ne[0] == MNIST_NCLASSES);
  MML_ASSERT(model.logits->ne[1] == model.nbatch_physical);
  MML_ASSERT(model.logits->ne[2] == 1);
  MML_ASSERT(model.logits->ne[3] == 1);
}
...
{
  ggml_opt_fit(model.backend_sched,
               model.ctx_compute,
               model.images,
               model.logits,
               dataset,
...
void mnist_model_save(mnist_model& model, const std::string& fname)
{
  ...
  std::vector<struct MmlTensor*> weights;
  if (model.arch == "mnist-fc")
  {
  ...
  {
    weights = {
      model.conv1_kernel, model.conv1_bias, model.conv2_kernel, model.conv2_bias, model.dense_weight, model.dense_bias
    };
  }
  else
  {
  ...
  for (struct MmlTensor* t : weights)
  {
    struct MmlTensor* copy = ggml_dup_tensor(ggml_ctx, t);
...
int wasm_eval(uint8_t* digitPtr)
{
  ...
  struct MmlTensor* data = ggml_opt_dataset_data(dataset);
  ...
  mnist_model model = mnist_model_init_from_file("mnist-f32.gguf", "CPU", /*nbatch_logical =*/ 1, /*nbatch_physical =*/
                                                 1);
