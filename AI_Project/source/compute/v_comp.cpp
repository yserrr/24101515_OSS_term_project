...
struct v_tensor* v_set_zero(struct v_tensor* tensor) {
  if (is_empty(tensor)) {
    return tensor;
  }
  if (tensor->buffer) {
...
struct v_tensor* v_get_rows(struct v_ctx* ctx,
                            struct v_tensor* a,
                            struct v_tensor* b) {
  ...
  struct v_tensor* result = v_new_tensor_4d(ctx, type, a->ne[0], b->ne[0], b->ne[1], b->ne[2]);
...
struct v_tensor* v_get_rows_back(struct v_ctx* ctx,
                                    struct v_tensor* a,
                                    struct v_tensor* b,
                                    struct v_tensor* c) {
  ...
  struct v_tensor* result = v_new_tensor_2d(ctx, v_TYPE_F32, c->ne[0], c->ne[1]);
...
struct v_tensor* v_diag(struct v_ctx* ctx,
  ...
  struct v_tensor* result = v_new_tensor(ctx, a->type, 4, ne);
...
struct v_tensor* v_out_prod(struct v_ctx* ctx,
                            struct v_tensor* a,
                            struct v_tensor* b) {
...
struct v_tensor* v_glu_impl(struct v_ctx* ctx,
  ...
  int64_t ne[MML_MAX_DIMS] = {a->ne[0] / 2};
  for (int i = 1; i < MML_MAX_DIMS; i++) ne[i] = a->ne[i];
  struct v_tensor* result = new_tensor_impl(ctx, a->type, MML_MAX_DIMS, b
                                                                          ? a->ne
                                                                          : ne, NULL, 0);
...
struct v_tensor* v_repeat_4d(struct v_ctx* ctx,
                                struct v_tensor* a,
                                int64_t ne0,
                                int64_t ne1,
                                int64_t ne2,
...
struct v_tensor* v_permute(struct v_ctx* ctx,
                           struct v_tensor* a,
                           int axis0,
                           int axis1,
                           int axis2,
                           int axis3) {
  V_ASSERT(axis0 >= 0 && axis0 < MML_MAX_DIMS);
  V_ASSERT(axis1 >= 0 && axis1 < MML_MAX_DIMS);
  V_ASSERT(axis2 >= 0 && axis2 < MML_MAX_DIMS);
  V_ASSERT(axis3 >= 0 && axis3 < MML_MAX_DIMS);
...
struct v_tensor* v_set_rows(struct v_ctx* ctx,
                            struct v_tensor* a,
                            struct v_tensor* b,
                            struct v_tensor* c) {
...
struct v_tensor* v_transpose(struct v_ctx* ctx,
                             struct v_tensor* a) {
  struct v_tensor* result = v_tensor_view(ctx, a);
...
struct v_tensor* v_sum_rows(struct v_ctx* ctx,
                               struct v_tensor* a) {
  int64_t ne[MML_MAX_DIMS] = {1};
  for (int i = 1; i < MML_MAX_DIMS; ++i) {
    ne[i] = a->ne[i];
  }

  struct v_tensor* result = v_new_tensor(ctx, a->type, MML_MAX_DIMS, ne);
...
struct v_tensor* v_mean(struct v_ctx* ctx,
                           struct v_tensor* a) {
...
struct v_tensor* v_sum(struct v_ctx* ctx,
                       struct v_tensor* a) {
...
struct v_tensor* add_cast_impl(struct v_ctx* ctx,
  ...
  struct v_tensor* result = v_new_tensor(ctx, type, MML_MAX_DIMS, a->ne);
...
struct v_tensor* v_matmul(struct v_ctx* ctx,
                          struct v_tensor* a,
                          struct v_tensor* b) {
...
struct v_tensor* add_id(struct v_ctx* ctx,
                        struct v_tensor* a,
                        struct v_tensor* b,
                        struct v_tensor* ids) {
  V_ASSERT(a->ne[0] == b->ne[0]);
...
struct v_tensor* v_argmax(struct v_ctx* ctx,
                          struct v_tensor* a) {
  V_ASSERT(v_is_matrix(a));
  V_ASSERT(a->ne[0] <= INT32_MAX);

  struct v_tensor* result = v_new_tensor_1d(ctx, v_TYPE_I32, a->ne[1]);
  result->op              = V_OP_ARGMAX;
  result->src[0]          = a;
  return result;
}

struct v_tensor* v_count_equal(struct v_ctx* ctx,
                                  struct v_tensor* a,
                                  struct v_tensor* b) {
  V_ASSERT(v_are_same_shape(a, b));

  struct v_tensor* result = v_new_tensor_1d(ctx, v_TYPE_I64, 1);
...
struct v_tensor* v_repeat(struct v_ctx* ctx,
                             struct v_tensor* a,
                             struct v_tensor* b) {
  V_ASSERT(can_repeat(a, b));

  struct v_tensor* result = v_new_tensor(ctx, a->type, MML_MAX_DIMS, b->ne);
...
struct v_tensor* v_repeat_back(struct v_ctx* ctx,
                                  struct v_tensor* a,
                                  struct v_tensor* b) {
  V_ASSERT(can_repeat(b, a));
...
struct v_tensor* v_concat(
  struct v_ctx* ctx,
  struct v_tensor* a,
  struct v_tensor* b,
  int dim) {
  V_ASSERT(dim >= 0 && dim < MML_MAX_DIMS);
  V_ASSERT(a->type == b->type);

  int64_t ne[MML_MAX_DIMS];
  for (int d = 0; d < MML_MAX_DIMS; ++d) {
    if (d == dim) {
      ne[d] = a->ne[d] + b->ne[d];
      continue;
    }
...
struct v_tensor* v_leaky_relu(struct v_ctx* ctx,
                                 struct v_tensor* a,
...
struct v_tensor* v_xielu(struct v_ctx* ctx,
                            struct v_tensor* a,
                            float alpha_n,
                            float alpha_p,
                            float beta,
...
struct v_tensor* v_swiglu_oai(struct v_ctx* ctx,
                                 struct v_tensor* a,
                                 struct v_tensor* b,
                                 float alpha,
                                 float limit) {
  struct v_tensor* result = v_glu_impl(ctx, a, b, v_GLU_OP_SWIGLU_OAI, false);
...
struct v_tensor* v_rms_norm_back(struct v_ctx* ctx,
                                    struct v_tensor* a,
                                    struct v_tensor* b,
                                    float eps) {
  struct v_tensor* result = v_dup_tensor(ctx, a);
...
*/
struct v_tensor* mmlMatrixMulId(struct v_ctx* ctx,
                                struct v_tensor* as,
                                struct v_tensor* b,
                                struct v_tensor* ids) {
...
struct v_tensor* v_cast(struct v_ctx* ctx,
                        struct v_tensor* a,
                        enum v_data_type type) {
  struct v_tensor* result = v_new_tensor(ctx, type, MML_MAX_DIMS, a->ne);
...
struct v_tensor* v_mem_cont(struct v_ctx* ctx,
                            struct v_tensor* a) {
  struct v_tensor* result = v_dup_tensor(ctx, a);
