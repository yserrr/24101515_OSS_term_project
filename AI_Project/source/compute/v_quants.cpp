...
void quantize_row_q4_0_ref(const float* v_RESTRICT x, block_q4_0* v_RESTRICT y, int64_t k)
{
...
void quantize_row_q4_1_ref(const float* v_RESTRICT x, block_q4_1* v_RESTRICT y, int64_t k)
{
  const int qk = QK4_1;
...
void quantize_row_q5_0_ref(const float* v_RESTRICT x, block_q5_0* v_RESTRICT y, int64_t k)
{
...
void quantize_row_q5_1_ref(const float* v_RESTRICT x, block_q5_1* v_RESTRICT y, int64_t k)
{
  const int qk = QK5_1;
...
// reference implementation for deterministic creation of model files
void quantize_row_q8_0_ref(const float* v_RESTRICT x, block_q8_0* v_RESTRICT y, int64_t k)
{
  assert(k % QK8_0 == 0);
...
// reference implementation for deterministic creation of model files
void quantize_row_q8_1_ref(const float* v_RESTRICT x, block_q8_1* v_RESTRICT y, int64_t k)
{
  assert(QK8_1 == 32);
  assert(k % QK8_1 == 0);
...
void quantize_row_mxfp4_ref(const float* v_RESTRICT x, block_mxfp4* v_RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    const float d = v_E8M0_TO_FP32_HALF(e);
...
void dequantize_row_q4_0(const block_q4_0* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  static const int qk = QK4_0;
  ...
  for (int i = 0; i < nb; i++)
  {
    const float d = v_FP16_TO_FP32(x[i].d);
...
void dequantize_row_q4_1(const block_q4_1* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  ...
  {
    const float d = v_FP16_TO_FP32(x[i].d);
    const float m = v_FP16_TO_FP32(x[i].m);
...
void dequantize_row_q5_0(const block_q5_0* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  static const int qk = QK5_0;
...
void dequantize_row_q5_1(const block_q5_1* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
...
void dequantize_row_q8_0(const block_q8_0* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
...
void dequantize_row_mxfp4(const block_mxfp4* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  static const int qk = QK_MXFP4;
  ...
  for (int i = 0; i < nb; i++)
  {
    const float d = v_E8M0_TO_FP32_HALF(x[i].e);
...
static float make_qx_quants(int n, int nmax, const float* v_RESTRICT x, int8_t* v_RESTRICT L, int rmse_type,
                            const float* v_RESTRICT qw)
{
  float max = 0;
  float amax = 0;
  for (int i = 0; i < n; ++i)
...
static float make_q3_quants(int n, int nmax, const float* v_RESTRICT x, int8_t* v_RESTRICT L, bool do_rmse)
{
  float max = 0;
  float amax = 0;
  for (int i = 0; i < n; ++i)
...
static float make_qkx1_quants(int n, int nmax, const float* v_RESTRICT x, uint8_t* v_RESTRICT L,
                              float* v_RESTRICT the_min,
...
static float make_qkx2_quants(int n, int nmax, const float* v_RESTRICT x, const float* v_RESTRICT weights,
                              uint8_t* v_RESTRICT L, float* v_RESTRICT the_min, uint8_t* v_RESTRICT Laux,
...
static inline void get_scale_min_k4(int j, const uint8_t* v_RESTRICT q, uint8_t* v_RESTRICT d,
                                    uint8_t* v_RESTRICT m)
{
  if (j < 4)
  {
    *d = q[j] & 63;
...
void quantize_row_q2_K_ref(const float* v_RESTRICT x, block_q2_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    if (max_scale > 0)
    {
      ...
      y[i].d = v_FP32_TO_FP16(max_scale/q4scale);
    }
    else
    {
      for (int j = 0; j < QK_K / 16; ++j) y[i].scales[j] = 0;
      y[i].d = v_FP32_TO_FP16(0.f);
    }
    if (max_min > 0)
    {
      ...
      y[i].dmin = v_FP32_TO_FP16(max_min/q4scale);
    }
    else
    {
      y[i].dmin = v_FP32_TO_FP16(0.f);
    }
    for (int j = 0; j < QK_K / 16; ++j)
    {
      const float d = v_FP16_TO_FP32(y[i].d) * (y[i].scales[j] & 0xF);
      if (!d) continue;
      const float dm = v_FP16_TO_FP32(y[i].dmin) * (y[i].scales[j] >> 4);
...
void dequantize_row_q2_K(const block_q2_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  {
    const float d = v_FP16_TO_FP32(x[i].d);
    const float min = v_FP16_TO_FP32(x[i].dmin);
...
static float make_qkx3_quants(int n, int nmax, const float* v_RESTRICT x, const float* v_RESTRICT weights,
                              uint8_t* v_RESTRICT L, float* v_RESTRICT the_min, uint8_t* v_RESTRICT Laux,
                              float rmin, float rdelta, int nstep, bool use_mad)
{
  float min = x[0];
...
static float make_qp_quants(int n, int nmax, const float* v_RESTRICT x, uint8_t* v_RESTRICT L,
...
static void quantize_row_q2_K_impl(const float* v_RESTRICT x, block_q2_K* v_RESTRICT y, int k,
                                   const float* v_RESTRICT quant_weights)
{
  V_ASSERT(quant_weights);
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      const float* v_RESTRICT qw = quant_weights + QK_K * i + 16 * j;
    ...
    y[i].d = v_FP32_TO_FP16(dm);
    y[i].dmin = v_FP32_TO_FP16(mm);
    dm = v_FP16_TO_FP32(y[i].d);
    mm = v_FP16_TO_FP32(y[i].dmin);
...
size_t quantize_q2_K(const float* src, block_q2_K* dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  size_t row_size = v_row_size(v_TYPE_Q2_K, n_per_row);
  if (!quant_weights)
  {
...
void quantize_row_q3_K_ref(const float* v_RESTRICT x, block_q3_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      ...
      float d = v_FP16_TO_FP32(y[i].d) * sc;
      if (!d)
      {
        continue;
      }
...
void dequantize_row_q3_K(const block_q3_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    const float d_all = v_FP16_TO_FP32(x[i].d);

    const uint8_t* v_RESTRICT q = x[i].qs;
    const uint8_t* v_RESTRICT hm = x[i].hmask;
    uint8_t m = 1;
...
static void quantize_row_q3_K_impl(const float* v_RESTRICT x, block_q3_K* v_RESTRICT y, int64_t n_per_row,
                                   const float* v_RESTRICT quant_weights)
{
...
size_t quantize_q3_K(const float* v_RESTRICT src,
                     void* v_RESTRICT dst,
                     int64_t nrow,
                     int64_t n_per_row,
                     const float* quant_weights)
{
  size_t row_size = v_row_size(v_TYPE_Q3_K, n_per_row);
  if (!quant_weights)
  {
...
void quantize_row_q4_K_ref(const float* v_RESTRICT x, block_q4_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    y[i].d = v_FP32_TO_FP16(max_scale/63.f);
    y[i].dmin = v_FP32_TO_FP16(max_min/63.f);

    uint8_t sc, m;
    for (int j = 0; j < QK_K / 32; ++j)
    {
      get_scale_min_k4(j, y[i].scales, &sc, &m);
      const float d = v_FP16_TO_FP32(y[i].d) * sc;
      if (!d) continue;
      const float dm = v_FP16_TO_FP32(y[i].dmin) * m;
...
void dequantize_row_q4_K(const block_q4_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
...
static void quantize_row_q4_K_impl(const float* v_RESTRICT x, block_q4_K* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
  assert(n_per_row % QK_K == 0);
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    y[i].d = v_FP32_TO_FP16(d_block);
    y[i].dmin = v_FP32_TO_FP16(m_block);
...
size_t quantize_q4_K(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  size_t row_size = v_row_size(v_TYPE_Q4_K, n_per_row);
  if (!quant_weights)
  {
...
void quantize_row_q5_K_ref(const float* v_RESTRICT x, block_q5_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    uint8_t* v_RESTRICT qh = y[i].qh;
    uint8_t* v_RESTRICT ql = y[i].qs;
    memset(qh, 0, QK_K / 8);
...
void dequantize_row_q5_K(const block_q5_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
static void quantize_row_q5_K_impl(const float* v_RESTRICT x, block_q5_K* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
  assert(n_per_row % QK_K == 0);
  const int64_t nb = n_per_row / QK_K;
...
size_t quantize_q5_K(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  size_t row_size = v_row_size(v_TYPE_Q5_K, n_per_row);
  if (!quant_weights)
  {
...
void quantize_row_q6_K_ref(const float* v_RESTRICT x, block_q6_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      float d = v_FP16_TO_FP32(y[i].d) * y[i].scales[j];
      if (!d)
      {
        continue;
      }
...
void dequantize_row_q6_K(const block_q6_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    const uint8_t* v_RESTRICT ql = x[i].ql;
    const uint8_t* v_RESTRICT qh = x[i].qh;
    const int8_t* v_RESTRICT sc = x[i].scales;
...
static void quantize_row_q6_K_impl(const float* v_RESTRICT x, block_q6_K* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
  assert(n_per_row % QK_K == 0);
  const int64_t nb = n_per_row / QK_K;
...
size_t quantize_q6_K(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  size_t row_size = v_row_size(v_TYPE_Q6_K, n_per_row);
  if (!quant_weights)
  {
...
static void quantize_row_q4_0_impl(const float* v_RESTRICT x, block_q4_0* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
...
size_t quantize_q4_0(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  if (!quant_weights)
  {
    quantize_row_q4_0_ref(src, (block_q4_0*)dst, (int64_t)nrow * n_per_row);
    return nrow * v_row_size(v_TYPE_Q4_0, n_per_row);
  }
  size_t row_size = v_row_size(v_TYPE_Q4_0, n_per_row);
  char* qrow = (char*)dst;
  for (int64_t row = 0; row < nrow; ++row)
  {
...
static void quantize_row_q4_1_impl(const float* v_RESTRICT x, block_q4_1* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
...
size_t quantize_q4_1(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  if (!quant_weights)
  {
    quantize_row_q4_1_ref(src, (block_q4_1*)dst, (int64_t)nrow * n_per_row);
    return nrow * v_row_size(v_TYPE_Q4_1, n_per_row);
  }
  size_t row_size = v_row_size(v_TYPE_Q4_1, n_per_row);
  char* qrow = (char*)dst;
  for (int64_t row = 0; row < nrow; ++row)
  {
...
static void quantize_row_q5_0_impl(const float* v_RESTRICT x, block_q5_0* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
...
size_t quantize_q5_0(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  if (!quant_weights)
  {
    quantize_row_q5_0_ref(src, (block_q5_0*)dst, (int64_t)nrow * n_per_row);
    return nrow * v_row_size(v_TYPE_Q5_0, n_per_row);
  }
  size_t row_size = v_row_size(v_TYPE_Q5_0, n_per_row);
  char* qrow = (char*)dst;
  for (int64_t row = 0; row < nrow; ++row)
  {
...
static void quantize_row_q5_1_impl(const float* v_RESTRICT x, block_q5_1* v_RESTRICT y, int64_t n_per_row,
                                   const float* quant_weights)
{
...
size_t quantize_q5_1(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                     const float* quant_weights)
{
  if (!quant_weights)
  {
    quantize_row_q5_1_ref(src, (block_q5_1*)dst, (int64_t)nrow * n_per_row);
    return nrow * v_row_size(v_TYPE_Q5_1, n_per_row);
  }
  size_t row_size = v_row_size(v_TYPE_Q5_1, n_per_row);
  char* qrow = (char*)dst;
  for (int64_t row = 0; row < nrow; ++row)
  {
...
void quantize_row_tq1_0_ref(const float* v_RESTRICT x, block_tq1_0* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void quantize_row_tq2_0_ref(const float* v_RESTRICT x, block_tq2_0* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_tq1_0(const block_tq1_0* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_tq2_0(const block_tq2_0* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq2_xxs(const block_iq2_xxs* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq2_xs(const block_iq2_xs* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq2_s(const block_iq2_s* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq3_xxs(const block_iq3_xxs* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq3_s(const block_iq3_s* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_iq1_s(const block_iq1_s* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  //  assert(k % QK_K == 0);
  //  const int64_t nb = k / QK_K;
  //
...
void dequantize_row_iq1_m(const block_iq1_m* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    const float d = v_FP16_TO_FP32(scale.f16);
...
void dequantize_row_iq4_nl(const block_iq4_nl* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK4_NL == 0);
...
void dequantize_row_iq4_xs(const block_iq4_xs* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void quantize_row_q8_K_ref(const float* v_RESTRICT x, block_q8_K* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
void dequantize_row_q8_K(const block_q8_K* v_RESTRICT x, float* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  const int64_t nb = k / QK_K;
...
static inline int iq2_data_index(enum v_data_type type)
{
  V_ASSERT(
    type == v_TYPE_IQ2_XXS || type == v_TYPE_IQ2_XS || type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M || type
    == v_TYPE_IQ2_S);
  return type == v_TYPE_IQ2_XXS
           ? 0
           : type == v_TYPE_IQ2_XS
           ? 1
           : type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M
           ? 2
           : 3;
}

static inline int iq2_grid_size(enum v_data_type type)
{
  V_ASSERT(
    type == v_TYPE_IQ2_XXS || type == v_TYPE_IQ2_XS || type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M || type
    == v_TYPE_IQ2_S);
...
void iq2xs_init_impl(enum v_data_type type)
{
  ...
  //const int nwant = type == v_TYPE_IQ1_S ? 3 : 2;
  const int nwant = type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M ? 3 : type == v_TYPE_IQ2_S ? 1 : 2;
  const uint16_t* kgrid = type == v_TYPE_IQ2_XXS
                            ? kgrid_2bit_256
                            : type == v_TYPE_IQ2_XS
                            ? kgrid_2bit_512
                            : type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M
...
void iq2xs_free_impl(enum v_data_type type)
{
  V_ASSERT(
    type == v_TYPE_IQ2_XXS || type == v_TYPE_IQ2_XS || type == v_TYPE_IQ1_S || type == v_TYPE_IQ1_M || type
    == v_TYPE_IQ2_S);
...
static int iq2_find_best_neighbour(const uint16_t* v_RESTRICT neighbours, const uint64_t* v_RESTRICT grid,
                                   const float* v_RESTRICT xval, const float* v_RESTRICT weight, float scale,
                                   int8_t* v_RESTRICT L)
{
  int num_neighbors = neighbours[0];
  V_ASSERT(num_neighbors > 0);
  ...
  V_ASSERT(grid_index >= 0);
  const int8_t* pg = (const int8_t*)(grid + grid_index);
  for (int i = 0; i < 8; ++i) L[i] = (pg[i] - 1) / 2;
  return grid_index;
}

static void quantize_row_iq2_xxs_impl(const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                      const float* v_RESTRICT quant_weights)
{
  const int gindex = iq2_data_index(v_TYPE_IQ2_XXS);

  const uint64_t* kgrid_q2xs = iq2_data[gindex].grid;
  const int* kmap_q2xs = iq2_data[gindex].map;
  const uint16_t* kneighbors_q2xs = iq2_data[gindex].neighbours;

  V_ASSERT(quant_weights && "missing quantization weights");
  V_ASSERT(kgrid_q2xs && "forgot to call v_quantize_init()?");
  V_ASSERT(kmap_q2xs && "forgot to call v_quantize_init()?");
  V_ASSERT(kneighbors_q2xs && "forgot to call v_quantize_init()?");
  V_ASSERT(n%QK_K == 0);
      ...
      for (int is = -6; is <= 6; ++is)
      {
        ...
        for (int k = 0; k < 4; ++k)
        {
          ...
          if (grid_index < 0)
          {
            ...
            grid_index = iq2_find_best_neighbour(neighbours,
                                                 kgrid_q2xs,
                                                 xval + 8 * k,
                                                 waux + 8 * k,
                                                 this_scale,
...
static void quantize_row_iq2_xs_impl(const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                     const float* v_RESTRICT quant_weights)
{
  const int gindex = iq2_data_index(v_TYPE_IQ2_XS);
...
size_t quantize_iq2_xxs(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                        const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
...
size_t quantize_iq2_xs(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                       const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
  int64_t nblock = n_per_row / QK_K;
...
{
  (void)grid_size;
  V_ASSERT(grid_size == 256 || grid_size == 512);
...
static int iq3_find_best_neighbour(const uint16_t* v_RESTRICT neighbours, const uint32_t* v_RESTRICT grid,
                                   const float* v_RESTRICT xval, const float* v_RESTRICT weight, float scale,
                                   int8_t* v_RESTRICT L)
{
  int num_neighbors = neighbours[0];
...
static void quantize_row_iq3_xxs_impl(int grid_size, const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                      const float* v_RESTRICT quant_weights)
{
  const int gindex = iq3_data_index(grid_size);
  ...
  //v_ASSERT(quant_weights   && "missing quantization weights");
  V_ASSERT(kgrid_q3xs && "forgot to call v_quantize_init()?");
  V_ASSERT(kmap_q3xs && "forgot to call v_quantize_init()?");
  V_ASSERT(kneighbors_q3xs && "forgot to call v_quantize_init()?");
  V_ASSERT(n%QK_K == 0);
  ...
  int quant_size = block_size - sizeof(v_fp16_t);
  ...
  for (int ibl = 0; ibl < nbl; ++ibl)
  {
      ...
      for (int is = -15; is <= 15; ++is)
      {
        ...
        for (int k = 0; k < 8; ++k)
        {
          ...
          if (grid_index < 0)
          {
            ...
            grid_index = iq3_find_best_neighbour(neighbours,
                                                 kgrid_q3xs,
                                                 xval + 4 * k,
                                                 waux + 4 * k,
                                                 this_scale,
    ...
    if (!max_scale)
    {
      ...
      dh += block_size / sizeof(v_fp16_t);
      qs += block_size;
      continue;
    }

    float d = max_scale / 31;
    dh[0] = v_FP32_TO_FP16(d * 1.0125f); // small improvement via this fudge factor
    float id = 1 / d;
    for (int ib = 0; ib < QK_K / 32; ++ib)
    {
      int l = nearest_int(0.5f * (id * scales[ib] - 1));
...
size_t quantize_iq3_xxs(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                        const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
  int64_t nblock = n_per_row / QK_K;
...
void quantize_row_iq3_xxs_ref(const float* v_RESTRICT x, block_iq3_xxs* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
  quantize_row_iq3_xxs_impl(256, x, y, k, NULL);
}

static void quantize_row_iq3_s_impl(int block_size, const float* v_RESTRICT x, void* v_RESTRICT vy, int n,
                                    const float* v_RESTRICT quant_weights,
                                    float* scales,
                                    float* weight,
                                    float* xval,
                                    ...
                                    uint8_t* block_signs)
{
  ...
  for (int ibl = 0; ibl < nbl; ++ibl)
  {
    ...
    float d = max_scale / 31;
    y[ibl].d = v_FP32_TO_FP16(d * 1.033f);
    float id = 1 / d;
...
size_t quantize_iq3_s(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                      const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
  int64_t nblock = n_per_row / QK_K;
  ...
  for (int64_t row = 0; row < nrow; ++row)
  {
    quantize_row_iq3_s_impl(IQ3S_BLOCK_SIZE,
                            src,
                            qrow,
                            n_per_row,
                            quant_weights,
...
void quantize_row_iq3_s_ref(const float* v_RESTRICT x, block_iq3_s* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
...
static int iq1_find_best_neighbour(const uint16_t* v_RESTRICT neighbours, const uint64_t* v_RESTRICT grid,
                                   const float* v_RESTRICT xval, const float* v_RESTRICT weight, float* scale,
                                   int8_t* v_RESTRICT L, int ngrid)
{
  int num_neighbors = neighbours[0];
  V_ASSERT(num_neighbors > 0);
...
static int iq1_find_best_neighbour2(const uint16_t* v_RESTRICT neighbours, const uint64_t* v_RESTRICT grid,
                                    const float* v_RESTRICT xval, const float* v_RESTRICT weight, float scale,
                                    const float* v_RESTRICT xg, int8_t* v_RESTRICT L, int ngrid)
{
  int num_neighbors = neighbours[0];
  V_ASSERT(num_neighbors > 0);
  float best_score = FLT_MAX;
...
static void quantize_row_iq1_s_impl(const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                    const float* v_RESTRICT quant_weights,
                                    float* scales,
                                    float* weight,
                                    float* sumx,
                                    ...
                                    int8_t* shifts)
{
  const int gindex = iq2_data_index(v_TYPE_IQ1_S);
  ...
  for (int ibl = 0; ibl < nbl; ++ibl)
  {
    ...
    for (int ib = 0; ib < QK_K / block_size; ++ib)
    {
      ...
      for (int i1 = 0; i1 <= block_size; ++i1)
      {
        ...
        {
          float sumqx = (sumx[i1] - sumx[0]) * x_p[0] + (sumx[i2] - sumx[i1]) * x_p[1] + (sumx[block_size] - sumx[i2]) *
                        x_p[2];
          ...
          sumqx = (sumx[i1] - sumx[0]) * x_m[0] + (sumx[i2] - sumx[i1]) * x_m[1] + (sumx[block_size] - sumx[i2]) * x_m[
                    2];
      ...
      V_ASSERT(besti1 >= 0 && besti2 >= 0 && best_shift != 0);
      ...
      for (int k = 0; k < block_size / 8; ++k)
      {
        ...
        if (grid_index < 0)
        {
          all_on_grid = false;
          const uint16_t* neighbours = kneighbors_q2xs - kmap_q2xs[u] - 1;
          grid_index = iq1_find_best_neighbour2(neighbours,
                                                kgrid_q2xs,
                                                xb + 8 * k,
                                                weight + 8 * k,
                                                scale,
    ...
    y[ibl].d = v_FP32_TO_FP16(d*1.125f); // 1.125f is another fudge factor. Don't ask me why it is needed.
    float id = 1 / d;
    for (int ib = 0; ib < QK_K / block_size; ++ib)
    {
      int l = nearest_int(0.5f * (id * scales[ib] - 1));
...
size_t quantize_iq1_s(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                      const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
...
static void quantize_row_iq1_m_impl(const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                    const float* v_RESTRICT quant_weights,
                                    float* scales,
                                    float* weight,
                                    float* pairs,
                                    ...
                                    int8_t* shifts)
{
  const int gindex = iq2_data_index(v_TYPE_IQ1_M);
  ...
  for (int ibl = 0; ibl < nbl; ++ibl)
  {
    ...
    for (int ib = 0; ib < QK_K / block_size; ++ib)
    {
      ...
      V_ASSERT(besti1 >= 0 && besti2 >= 0 && best_k >= 0);
      for (int j = 0; j < besti1; ++j) L[idx[2 * j]] = 0;
      for (int j = besti1; j < besti2; ++j) L[idx[2 * j]] = 1;
      for (int j = besti2; j < block_size; ++j) L[idx[2 * j]] = 2;
      if (scale < 0)
    ...
    s.f16 = v_FP32_TO_FP16(d*1.1125f); // 1.1125f is another fudge factor. Don't ask me why it is needed.
...
size_t quantize_iq1_m(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                      const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
...
static void quantize_row_iq4_nl_impl(const int super_block_size, const int block_size, const float* v_RESTRICT x,
                                     v_fp16_t* dh, uint8_t* q4, uint16_t* scales_h, uint8_t* scales_l,
...
size_t quantize_iq4_nl(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                       const float* quant_weights)
{
  V_ASSERT(n_per_row%QK4_NL == 0);
  ...
  for (int64_t row = 0; row < nrow; ++row)
  {
    ...
    for (int ibl = 0; ibl < nblock; ++ibl)
    {
      ...
      quantize_row_iq4_nl_impl(QK4_NL,
                               32,
                               src + QK4_NL * ibl,
                               &iq4[ibl].d,
                               iq4[ibl].qs,
...
//void quantize_row_iq4_nl_ref(const float * v_RESTRICT x, void * v_RESTRICT vy, int64_t k) {
void quantize_row_iq4_nl_ref(const float* v_RESTRICT x, block_iq4_nl* v_RESTRICT y, int64_t k)
{
  V_ASSERT(k%QK4_NL == 0);
...
size_t quantize_iq4_xs(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                       const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
  int64_t nblock = n_per_row / QK_K;
...
void quantize_row_iq4_xs_ref(const float* v_RESTRICT x, block_iq4_xs* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
...
static void quantize_row_iq2_s_impl(const float* v_RESTRICT x, void* v_RESTRICT vy, int64_t n,
                                    const float* v_RESTRICT quant_weights)
{
  const int gindex = iq2_data_index(v_TYPE_IQ2_S);
  ...
  for (int ibl = 0; ibl < nbl; ++ibl)
  {
    ...
    float d = max_scale / 31;
    y[ibl].d = v_FP32_TO_FP16(d * 0.9875f);
    float id = 1 / d;
    for (int ib = 0; ib < QK_K / 16; ++ib)
    {
      int l = nearest_int(0.5f * (id * scales[ib] - 1));
...
size_t quantize_iq2_s(const float* v_RESTRICT src, void* v_RESTRICT dst, int64_t nrow, int64_t n_per_row,
                      const float* quant_weights)
{
  V_ASSERT(n_per_row%QK_K == 0);
  int64_t nblock = n_per_row / QK_K;
...
void quantize_row_iq2_s_ref(const float* v_RESTRICT x, block_iq2_s* v_RESTRICT y, int64_t k)
{
  assert(k % QK_K == 0);
...
static bool validate_float(float f, size_t i)
{
  if (isinf(f))
  {
    fprintf(stderr, "v_validate_row_data: found inf value at block %zu\n", i);
    return false;
  }

  if (isnan(f))
  {
    fprintf(stderr, "v_validate_row_data: found nan value at block %zu\n", i);
    return false;
  }
...
static bool isinf_fp16(v_fp16_t f)
{
  return (f & 0x7c00) == 0x7c00 && (f & 0x03ff) == 0;
}

static bool isnan_fp16(v_fp16_t f)
{
  return (f & 0x7c00) == 0x7c00 && (f & 0x03ff) != 0;
}

static bool validate_fp16(v_fp16_t f, size_t i)
{
  if (isinf_fp16(f))
  {
    fprintf(stderr, "v_validate_row_data: found inf value at block %zu\n", i);
...
static bool validate_e_e8m0(uint8_t e, size_t i)
{
  if (e == 0xff)
  {
    fprintf(stderr, "v_validate_row_data: found invalid e value %d at block %zu\n", e, i);
    return false;
  }
...
bool v_validate_row_data(enum v_data_type type, const void* data, size_t nbytes)
{
  ...
  const size_t nb = nbytes / v_type_size(type);
  ...
  case v_TYPE_F16:
    {
      const v_fp16_t* f = (const v_fp16_t*)data;
      size_t i = 0;
#if defined(__AVX2__)
      for (; i + 15 < nb; i += 16)
      {
