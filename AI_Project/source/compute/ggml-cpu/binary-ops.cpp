...
template <float (*op)(float, float), typename src0_t, typename src1_t, typename dst_t>
static void apply_binary_op(const ggml_compute_params * params, MlTensor * dst) {
    const MlTensor * src0 = dst->src[0];
    const MlTensor * src1 = dst->src[1];

    MML_ASSERT(ggml_can_repeat(src1, src0) && ggml_are_same_shape(src0, dst));
...
static void binary_op(const ggml_compute_params * params, MlTensor * dst) {
    const MlTensor * src0 = dst->src[0];
    const MlTensor * src1 = dst->src[1];
...
void ggml_compute_forward_add_non_quantized(const ggml_compute_params * params, MlTensor * dst) {
    binary_op<op_add>(params, dst);
}

void ggml_compute_forward_sub(const ggml_compute_params * params, MlTensor * dst) {
    binary_op<op_sub>(params, dst);
}

void ggml_compute_forward_mul(const ggml_compute_params * params, MlTensor * dst) {
    binary_op<op_mul>(params, dst);
}

void ggml_compute_forward_div(const ggml_compute_params * params, MlTensor * dst) {
