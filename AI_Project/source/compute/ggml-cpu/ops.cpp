...
static void ggml_compute_forward_dup_same_cont(
    ...
    const MlTensor * src0 = dst->src[0];

    MML_ASSERT(ggml_nelements(dst) == ggml_nelements(src0));
    MML_ASSERT(ggml_is_contiguous(dst) && ggml_is_contiguous(src0));
    MML_ASSERT(src0->type == dst->type);
...
static void ggml_compute_forward_dup_flt(
    ...
    MML_ASSERT(ggml_nelements(dst) == ggml_nelements(src0));
    MML_ASSERT(!ggml_is_quantized(src0->type) && !ggml_is_quantized(dst->type));
...
static void ggml_compute_forward_dup_to_q(
    ...
    MML_ASSERT(ggml_nelements(dst) == ggml_nelements(src0));
    MML_ASSERT(!ggml_is_quantized(src0->type));
...
static void ggml_compute_forward_dup_from_q(
    ...
    const MlTensor * src0 = dst->src[0];
    const MlTensor * src1 = dst->src[1];

    GGML_TENSOR_BINARY_OP_LOCALS

    const MmlDataType type = src0->type;
    ...
    MML_ASSERT(nb10 == ggml_type_size(dst->type));
    // must either have first dimension large enough to hold a row, or fully contiguous
    MML_ASSERT((ne10 % qk) == 0 || ggml_is_contiguous(dst));
...
static void ggml_compute_forward_add_q_f32(
    ...
    MML_ASSERT(ggml_are_same_shape(src0, src1) && ggml_are_same_shape(src0, dst));
    ...
    const MmlDataType type = src0->type;
    const MmlDataType dtype = dst->type;
    ggml_to_float_t const dequantize_row_q = ggml_get_type_traits(type)->to_float;
    ggml_from_float_t const quantize_row_q = ggml_get_type_traits_cpu(dtype)->from_float;

    // we don't support permuted src0 or src1
    MML_ASSERT(nb00 == ggml_type_size(type));
    MML_ASSERT(nb10 == sizeof(float));
    ...
    MML_ASSERT(ggml_is_quantized(src0->type));
    MML_ASSERT(src1->type == GGML_TYPE_F32);
...
static void ggml_compute_forward_add_id_f32(
    ...
    const MlTensor * src0 = dst->src[0];
    const MlTensor * src1 = dst->src[1];
    const MlTensor * src2 = dst->src[2];

    MML_ASSERT(dst->type  == GGML_TYPE_F32);
    MML_ASSERT(src0->type == GGML_TYPE_F32);
    MML_ASSERT(src1->type == GGML_TYPE_F32);
    MML_ASSERT(src2->type == GGML_TYPE_I32);

    MML_ASSERT(src0->nb[0] == sizeof(float));
    MML_ASSERT(src1->nb[0] == sizeof(float));
...
static void ggml_compute_forward_add1_f32(
    ...
    MML_ASSERT(ggml_are_same_shape(src0, dst));
    MML_ASSERT(ggml_is_scalar(src1));
...
static void ggml_compute_forward_add1_f16_f32(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_F16);
    MML_ASSERT(src1->type == GGML_TYPE_F32);
    MML_ASSERT(dst->type  == GGML_TYPE_F16);

    MML_ASSERT( nb0 == sizeof(ggml_fp16_t));
    MML_ASSERT(nb00 == sizeof(ggml_fp16_t));
...
static void ggml_compute_forward_add1_f16_f16(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_F16);
    MML_ASSERT(src1->type == GGML_TYPE_F16);
    MML_ASSERT(dst->type  == GGML_TYPE_F16);
...
static void ggml_compute_forward_add1_bf16_f32(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_BF16);
    MML_ASSERT(src1->type == GGML_TYPE_F32);
    MML_ASSERT(dst->type  == GGML_TYPE_BF16);

    MML_ASSERT( nb0 == sizeof(ggml_bf16_t));
    MML_ASSERT(nb00 == sizeof(ggml_bf16_t));
...
static void ggml_compute_forward_add1_bf16_bf16(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_BF16);
    MML_ASSERT(src1->type == GGML_TYPE_BF16);
    MML_ASSERT(dst->type  == GGML_TYPE_BF16);
...
static void ggml_compute_forward_acc_f32(
    ...
    MML_ASSERT(offset + (ne10 == 0 ? 0 : ne10-1)*nb0  + (ne11 == 0 ? 0 : ne11-1)*nb1  + (ne12 == 0 ? 0 : ne12-1)*nb2  + (ne13 == 0 ? 0 : ne13-1)*nb3  < ggml_nbytes(dst));
    MML_ASSERT(offset + (ne10 == 0 ? 0 : ne10-1)*nb00 + (ne11 == 0 ? 0 : ne11-1)*nb01 + (ne12 == 0 ? 0 : ne12-1)*nb02 + (ne13 == 0 ? 0 : ne13-1)*nb03 < ggml_nbytes(src0));
...
static void ggml_compute_forward_sum_rows_f32(
    ...
    MML_ASSERT(src0->nb[0] == sizeof(float));
    MML_ASSERT(dst->nb[0] == sizeof(float));
...
static void ggml_compute_forward_count_equal_i32(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_I32);
    MML_ASSERT(src1->type == GGML_TYPE_I32);
    MML_ASSERT(ggml_are_same_shape(src0, src1));
    MML_ASSERT(ggml_is_scalar(dst));
    MML_ASSERT(dst->type == GGML_TYPE_I64);
...
static void ggml_compute_forward_repeat_f32(
    ...
    MML_ASSERT(ggml_can_repeat(src0, dst));
...
static void ggml_compute_forward_repeat_back_f32(
    ...
    MML_ASSERT(ggml_can_repeat(dst, src0));
...
static void ggml_compute_forward_concat_i8(
    ...
    MML_ASSERT(ggml_type_size(src0->type) == sizeof(int8_t));
...
static void ggml_compute_forward_concat_f16(
    ...
    MML_ASSERT(ggml_type_size(src0->type) == sizeof(ggml_fp16_t));
...
static void ggml_compute_forward_concat_f32(
    ...
    MML_ASSERT(ggml_type_size(src0->type) == sizeof(float));
...
static void ggml_compute_forward_reglu_f32(
    ...
    MML_ASSERT(ggml_is_contiguous_1(src0));
    MML_ASSERT(ggml_is_contiguous_1(dst));

    if (src1) {
        MML_ASSERT(ggml_is_contiguous_1(src1));
        MML_ASSERT(src0->type == src1->type);
    }
    ...
    MML_ASSERT(dst->ne[0] == nc);
    MML_ASSERT(ggml_nrows(dst) == nr);
...
static void ggml_compute_forward_rms_norm_back_f32(
    ...
    const MlTensor * src0 = dst->src[0]; // gradients from forward pass output
    const MlTensor * src1 = dst->src[1]; // src1 from forward pass

    MML_ASSERT(ggml_are_same_shape(src0, dst) && ggml_are_same_shape(src0, src1));
...
static void ggml_compute_forward_out_prod_f32(
    ...
    MML_ASSERT(ne2 % ne02 == 0);
    MML_ASSERT(ne3 % ne03 == 0);
...
static void ggml_compute_forward_out_prod_q_f32(
    ...
    MML_ASSERT(ne02 == ne12);
    MML_ASSERT(ne03 == ne13);
    MML_ASSERT(ne2  == ne12);
    MML_ASSERT(ne3  == ne13);
...
static void ggml_compute_forward_scale_f32(
    ...
    MML_ASSERT(ggml_is_contiguous(src0));
    MML_ASSERT(ggml_is_contiguous(dst));
    MML_ASSERT(ggml_are_same_shape(src0, dst));
...
static void ggml_compute_forward_set_f32(
    ...
    MML_ASSERT(offset + im0*nb0  + im1*nb1  + im2*nb2  + im3*nb3  <= ggml_nbytes(dst));
...
static void ggml_compute_forward_set_i32(
    ...
    MML_ASSERT(nb10 == sizeof(int32_t));
...
static void ggml_compute_forward_get_rows_back_f32_f16(
    ...
    MML_ASSERT( dst->ne[0] == nc);
    MML_ASSERT(src0->nb[0] == sizeof(ggml_fp16_t));
...
static void ggml_compute_forward_soft_max_ext_back_f32(
    ...
    MML_ASSERT(ggml_is_contiguous(src0));
    MML_ASSERT(ggml_is_contiguous(src1));
    MML_ASSERT(ggml_is_contiguous(dst));
    MML_ASSERT(ggml_are_same_shape(src0, dst));
    MML_ASSERT(ggml_are_same_shape(src1, dst));
...
static void ggml_compute_forward_rope_f32(
    ...
    MML_ASSERT(n_dims <= ne0);
    MML_ASSERT(n_dims % 2 == 0);
    ...
    if (is_mrope) {
        MML_ASSERT(sections[0] > 0 || sections[1] > 0 || sections[2] > 0);
    }

    if (is_vision) {
        MML_ASSERT(n_dims == ne0/2);
    }

    const float * freq_factors = NULL;
    if (src2 != NULL) {
        MML_ASSERT(src2->type == GGML_TYPE_F32);
        MML_ASSERT(src2->ne[0] >= n_dims / 2);
...
void ggml_compute_forward_im2col_back_f32(
    ...
    const MlTensor * src0 = dst->src[0]; // gradients of forward pass output
    const MlTensor * src1 = dst->src[1]; // convolution kernel
...
static void ggml_call_mul_mat(MmlDataType type, const ggml_compute_params * params, int64_t m, int64_t n, int64_t k,
                              void * a, void * b, float * c) {
    const ggml_type_traits * traits = ggml_get_type_traits(type);
    struct MlTensor src1 = {};
    src1.type  = type;
    src1.ne[0] = k;
    src1.ne[1] = m;
    src1.ne[2] = 1;
...
static void ggml_compute_forward_conv_2d_impl(const ggml_compute_params * params,
                                              const MlTensor *         kernel,  // [KW, KH, IC, OC]
                                              const MlTensor *         src,     // [W, H, C, N]
                                              MlTensor *               dst,     // [OW, OH, OC, N]
                                              MmlDataType                   kernel_type) {

    MML_ASSERT(ggml_is_contiguous(kernel));
    MML_ASSERT(kernel_type == GGML_TYPE_F16 || kernel_type == GGML_TYPE_F32);
    MML_ASSERT(kernel->type == kernel_type);
    ...
    MML_ASSERT(patches_per_batch > 0 && batch_size >= 1);
    ...
    for (int64_t batch_i = 0; batch_i < batch_n; ++batch_i) {
        ...
        MML_ASSERT(gemm_output + patch_n * c_out <= (float*)tmp + params->wsize);
...
void ggml_compute_forward_conv_2d_dw(
    ...
    const MlTensor * kernel = dst->src[0];
    const MlTensor * src = dst->src[1];
    ...
    MML_ASSERT(kernel->ne[3] == p.channels);
    MML_ASSERT(dst->ne[3] == p.batch);

    if (ggml_is_contiguous(src)) {
        ggml_compute_forward_conv_2d_dw_whcn(params, src, kernel, dst, p);
    } else if (ggml_is_contiguous_channels(src)) {
        // kernel should also have channels most contiguous in memory
        MML_ASSERT(kernel->nb[0] >= kernel->nb[2] && kernel->nb[1] >= kernel->nb[0]);
...
void ggml_compute_forward_pool_1d(
    ...
    const int s0 = opts[2];
    const int p0 = opts[3];
    MML_ASSERT(p0 == 0); // padding not supported
    MML_ASSERT(k0 == s0); // only s = k supported
...
void ggml_compute_forward_pool_2d_back(
    ...
    const MlTensor * src  = dst->src[0];
    const MlTensor * dstf = dst->src[1]; // forward tensor of dst
...
static void ggml_compute_forward_arange_f32(
    ...
    MML_ASSERT(ggml_nelements(dst) == steps);
...
static void ggml_compute_forward_flash_attn_ext_f16(
    ...
    const MlTensor * q     = dst->src[0];
    const MlTensor * k     = dst->src[1];
    const MlTensor * v     = dst->src[2];
    const MlTensor * mask  = dst->src[3];
    const MlTensor * sinks = dst->src[4];
    ...
    MML_ASSERT(nbq0 == ggml_type_size(q->type));
    MML_ASSERT(nbk0 == ggml_type_size(k->type));
    MML_ASSERT(nbv0 == ggml_type_size(v->type));
    ...
    MmlDataType         const k_vec_dot_type = ggml_get_type_traits_cpu(k->type)->vec_dot_type;
    ggml_from_float_t const q_to_vec_dot   = ggml_get_type_traits_cpu(k_vec_dot_type)->from_float;
    ggml_vec_dot_t    const kq_vec_dot     = ggml_get_type_traits_cpu(k->type)->vec_dot;
    ggml_to_float_t   const v_to_float     = ggml_get_type_traits(v->type)->to_float;

    MML_ASSERT((                            q_to_vec_dot) && "fattn: unsupported K-type");
    MML_ASSERT((v->type == GGML_TYPE_F32 || v_to_float  ) && "fattn: unsupported V-type");
...
static void ggml_compute_forward_flash_attn_back_f32(
    ...
    MML_ASSERT(nbq0 == sizeof(float));
    MML_ASSERT(nbk0 == sizeof(float));
    MML_ASSERT(nbv0 == sizeof(float));
    ...
    MmlDataType result_type = dst->type;
    MML_ASSERT(ggml_blck_size(result_type) == 1);
    const size_t tsize = ggml_type_size(result_type);

    const size_t offs_q = 0;
    const size_t offs_k = offs_q + MML_PAD(elem_q * tsize, GGML_MEM_ALIGN);
    const size_t offs_v = offs_k + MML_PAD(elem_k * tsize, GGML_MEM_ALIGN);
...
static void ggml_compute_forward_ssm_conv_f32(
        const ggml_compute_params * params,
        MlTensor * dst) {
    const MlTensor * src0 = dst->src[0]; // conv_x
    const MlTensor * src1 = dst->src[1]; // conv1d.weight
    ...
    MML_ASSERT( dst->ne[0] == nr);
    MML_ASSERT(src0->nb[0] == sizeof(float));
    MML_ASSERT(src1->nb[0] == sizeof(float));
    MML_ASSERT(src0->nb[1] == src0->ne[0]*sizeof(float));
...
static void ggml_compute_forward_ssm_scan_f32(
        const ggml_compute_params * params,
        MlTensor * dst) {
    const MlTensor * src0 = dst->src[0]; // s  {d_state, dim, n_head, n_seqs+}
    const MlTensor * src1 = dst->src[1]; // x  {dim, n_head, n_seq_tokens, n_seqs}
    const MlTensor * src2 = dst->src[2]; // dt {n_head, n_seq_tokens, n_seqs}
    const MlTensor * src3 = dst->src[3]; // A  {d_state, n_head} or {1, n_head}
    const MlTensor * src4 = dst->src[4]; // B  {d_state, n_group, n_seq_tokens, n_seqs}
    const MlTensor * src5 = dst->src[5]; // C  {d_state, n_group, n_seq_tokens, n_seqs}
    const MlTensor * src6 = dst->src[6]; // ids {n_seqs}
    ...
    MML_ASSERT(ggml_nelements(src1) + nc*nr*nh*ns == ggml_nelements(dst));
    MML_ASSERT(src0->nb[0] == sizeof(float));
    MML_ASSERT(src1->nb[0] == sizeof(float));
    MML_ASSERT(src2->nb[0] == sizeof(float));
    MML_ASSERT(src3->nb[0] == sizeof(float));
    MML_ASSERT(src4->nb[0] == sizeof(float));
    MML_ASSERT(src5->nb[0] == sizeof(float));
    MML_ASSERT(src6->nb[0] == sizeof(int32_t));
    MML_ASSERT(nh % ng == 0);
...
static void ggml_compute_forward_rwkv_wkv6_f32(
    ...
    MML_ASSERT(C % HEADS == 0); // C must be divisible by HEADS
...
static void ggml_compute_forward_cross_entropy_loss_f32(
    ...
    MML_ASSERT(src0->type == GGML_TYPE_F32);
    MML_ASSERT(src1->type == GGML_TYPE_F32);
    MML_ASSERT(src0->nb[0] == ggml_type_size(src0->type));
    MML_ASSERT(src1->nb[0] == ggml_type_size(src1->type));
    MML_ASSERT(ggml_are_same_shape(src0, src1));
    MML_ASSERT(ggml_is_scalar(dst));
    MML_ASSERT(dst->type == GGML_TYPE_F32);
    ...
    MML_ASSERT(params->wsize >= sizeof(float) * (nth + nth * nc));
...
static void ggml_compute_forward_cross_entropy_loss_back_f32(
    ...
    const MlTensor * grad  = dst->src[0]; // gradient of forward pass output
    const MlTensor * src0f = dst->src[1]; // src0 of forward pass
    const MlTensor * src1f = dst->src[2]; // src1 of forward pass

    MML_ASSERT(ggml_is_contiguous(dst));
    MML_ASSERT(ggml_is_contiguous(src0f));
    MML_ASSERT(ggml_is_contiguous(src1f));
    MML_ASSERT(ggml_is_contiguous(grad));
    MML_ASSERT(ggml_are_same_shape(src0f, src1f) && ggml_are_same_shape(src0f, dst));
...
static void ggml_compute_forward_opt_step_adamw_f32(
    ...
    const MlTensor * src0         = dst->src[0];
    const MlTensor * src0_grad    = dst->src[1];
    const MlTensor * src0_grad_m  = dst->src[2];
    const MlTensor * src0_grad_v  = dst->src[3];
    const MlTensor * adamw_params = dst->src[4];

    MML_ASSERT(ggml_are_same_shape(src0, src0_grad));
    MML_ASSERT(ggml_are_same_shape(src0, src0_grad_m));
    MML_ASSERT(ggml_are_same_shape(src0, src0_grad_v));
    MML_ASSERT(ggml_nelements(adamw_params) == 7);
    ...
    const float * adamw_params_ptr = mmlGetDataF32(adamw_params);
...
static void ggml_compute_forward_opt_step_sgd_f32(const ggml_compute_params * params, MlTensor * dst) {
    const MlTensor * src0       = dst->src[0];
    const MlTensor * src0_grad  = dst->src[1];
    const MlTensor * sgd_params = dst->src[2];

    MML_ASSERT(ggml_are_same_shape(src0, src0_grad));
    MML_ASSERT(ggml_nelements(sgd_params) == 2);
    ...
    const float * sgd_params_ptr   = mmlGetDataF32(sgd_params);
...
void ggml_compute_forward_opt_step_sgd(const ggml_compute_params * params, MlTensor * dst) {
    const MlTensor * src0 = dst->src[0];
