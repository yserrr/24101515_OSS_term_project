...
template <int64_t INTER_SIZE, MmlDataType PARAM_TYPE>
...
static int repack_q4_0_to_q4_0_4_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_Q4_0);
    MML_ASSERT(interleave_block == 4 || interleave_block == 8);
    ...
    MML_ASSERT(data_size == nrow * nblocks * sizeof(block_q4_0));
...
static int repack_q4_K_to_q4_K_8_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_Q4_K);
    MML_ASSERT(interleave_block == 8);
    ...
    MML_ASSERT(data_size == nrow * nblocks * sizeof(block_q4_K));
...
static int repack_q2_K_to_q2_K_8_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_Q2_K);
    MML_ASSERT(interleave_block == 8);
    constexpr int nrows_interleaved = 8;
    ...
    MML_ASSERT(data_size == nrow * nblocks * sizeof(block_q2_K));
...
static int repack_q4_0_to_q4_0_8_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_Q4_0);
    MML_ASSERT(interleave_block == 8);
    constexpr int nrows_interleaved = 8;
...
static int repack_iq4_nl_to_iq4_nl_4_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_IQ4_NL);
    MML_ASSERT(interleave_block == 4);
    ...
    MML_ASSERT(data_size == nrow * nblocks * sizeof(block_iq4_nl));
...
static int repack_iq4_nl_to_iq4_nl_8_bl(struct MlTensor * t, int interleave_block, const void * GGML_RESTRICT data, size_t data_size) {
    MML_ASSERT(t->type == GGML_TYPE_IQ4_NL);
    MML_ASSERT(interleave_block == 8);
...
int repack(struct MlTensor *, const void *, size_t);

// TODO: generalise.
template <> int repack<block_q4_0, 4, 4>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_q4_0_to_q4_0_4_bl(t, 4, data, data_size);
}

template <> int repack<block_q4_0, 8, 4>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_q4_0_to_q4_0_4_bl(t, 8, data, data_size);
}

template <> int repack<block_q4_0, 8, 8>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_q4_0_to_q4_0_8_bl(t, 8, data, data_size);
}

template <> int repack<block_q4_K, 8, 8>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_q4_K_to_q4_K_8_bl(t, 8, data, data_size);
}

template <> int repack<block_q2_K, 8, 8>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_q2_K_to_q2_K_8_bl(t, 8, data, data_size);
}

template <> int repack<block_iq4_nl, 4, 4>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_iq4_nl_to_iq4_nl_4_bl(t, 4, data, data_size);
}

// TODO: needs to be revisited
//template <> int repack<block_iq4_nl, 8, 4>(struct MmlTensor * t, const void * data, size_t data_size) {
//    return repack_iq4_nl_to_iq4_nl_4_bl(t, 8, data, data_size);
//}

template <> int repack<block_iq4_nl, 8, 8>(struct MlTensor * t, const void * data, size_t data_size) {
    return repack_iq4_nl_to_iq4_nl_8_bl(t, 8, data, data_size);
}

// gemv
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, MmlDataType PARAM_TYPE>
...
class tensor_traits_base : public ggml::cpu::tensor_traits {
  public:
    virtual int repack(struct MlTensor * t, const void * data, size_t data_size) = 0;
};

template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, MmlDataType PARAM_TYPE> class tensor_traits : public tensor_traits_base {

    bool work_size(int /* n_threads */, const struct MlTensor * op, size_t & size) override {
        ...
        switch (op->op) {
            ...
            case GGML_OP_MUL_MAT_ID:
                {
                    size = ggml_row_size(PARAM_TYPE, ggml_nelements(op->src[1]));
                    size = MML_PAD(size, sizeof(int64_t)); // + padding for next bloc.
    ...
    bool compute_forward(struct ggml_compute_params * params, struct MlTensor * op) override {
        switch (op->op) {
            case GGML_OP_MUL_MAT:
    ...
    void forward_mul_mat(ggml_compute_params * params, MlTensor * op) {
        const MlTensor * src0 = op->src[0];
        const MlTensor * src1 = op->src[1];
        MlTensor *       dst  = op;
        ...
        MML_ASSERT(ggml_n_dims(op->src[0]) == 2);
    ...
    void forward_mul_mat_id(ggml_compute_params * params, MlTensor * op) {
        const MlTensor * src0 = op->src[0];
        const MlTensor * src1 = op->src[1];
        const MlTensor * ids  = op->src[2];
        MlTensor *       dst  = op;
        ...
        MML_ASSERT(nb00 == ggml_type_size(src0->type));
        MML_ASSERT(nb10 == ggml_type_size(src1->type));
        ...
        MML_ASSERT(params->wsize >=
                (MML_PAD(nbw3, sizeof(int64_t)) +
                 n_as*(ne12 + 1)*sizeof(mmid_row_mapping))
                );

        auto * wdata          = (char *)params->wdata;
        auto * wdata_src1_end = (char *)wdata + MML_PAD(nbw3, sizeof(int64_t));
...
#undef MMID_MATRIX_ROW
    ...
    int repack(struct MlTensor * t, const void * data, size_t data_size) override {
...
static const ggml::cpu::tensor_traits * ggml_repack_get_optimal_repack_type(const struct MlTensor * cur) {
...
static enum ggml_status ggml_backend_cpu_repack_buffer_init_tensor(mml_backend_buffer_t buffer, struct MlTensor * tensor) {
...
static void ggml_backend_cpu_repack_buffer_set_tensor(mml_backend_buffer_t buffer, struct MlTensor * tensor,
                                                       const void * data, size_t offset, size_t size) {
    MML_ASSERT(offset == 0);
    MML_ASSERT(size == ggml_nbytes(tensor));
...
static mml_backend_buffer_t ggml_backend_cpu_repack_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    mml_backend_buffer_t buffer = mmlBackendBufferAllocate(ggml_backend_cpu_buffer_type(), size);
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
    bool supports_op(ggml_backend_dev_t, const struct MlTensor * op) override {
        if (    op->op == GGML_OP_MUL_MAT &&
            ...
            if (op->src[1]->buffer && !mmlBackendBufferIsHostVisible(op->src[1]->buffer->buft)) {
                return false;
            }
    ...
    ggml::cpu::tensor_traits * get_tensor_traits(const struct MlTensor * op) override {
...
ggml_backend_buffer_type_t ggml_backend_cpu_repack_buffer_type(void) {
    static struct MmlBackendBufferType ggml_backend_cpu_buffer_type_repack = {
        /* .iface    = */ {
