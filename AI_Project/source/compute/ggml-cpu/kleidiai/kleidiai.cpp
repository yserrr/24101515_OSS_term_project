...
static inline int64_t ggml_ne(const ggml_tensor * tensor, int dim) {
    MML_ASSERT(dim >= 0 && dim < GGML_MAX_DIMS);
...
class tensor_traits : public ggml::cpu::tensor_traits {
    ...
    bool compute_forward_fp16(ggml_compute_params * params, struct ggml_tensor * dst) {
        ...
        MML_ASSERT(rhs_batch_size0 > 0);
        MML_ASSERT(lhs_batch_size0 % rhs_batch_size0 == 0);
        const int64_t r = lhs_batch_size0 / rhs_batch_size0;
        ...
        MML_ASSERT(wsize_required <= params->wsize);
    ...
    bool compute_forward_q4_0(struct ggml_compute_params * params, struct ggml_tensor * dst) {
        MML_ASSERT(dst->src[0]->type == GGML_TYPE_Q4_0);
    ...
    bool compute_forward_get_rows(struct ggml_compute_params * params, struct ggml_tensor * dst) {
        ...
        for (int64_t i = ir0; i < ir1; ++i) {
            ...
            MML_ASSERT(row_idx >= 0 && row_idx < src0->ne[1]);
...
public:
    int repack(struct ggml_tensor * tensor, const void * data, size_t data_size) {
        MML_ASSERT(tensor->type == GGML_TYPE_Q4_0);
        MML_ASSERT(ctx.kernels);
