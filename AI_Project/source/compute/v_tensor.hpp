...
struct v_tensor {
  ...
  std::array<char, V_MAX_NAME> name;              // tensor name
  ...
  void set_outputs();
  void set_params();
  void set_mat_mul_precision(v_prec prec);
  //tensor states
  bool is_empty() const;
  ...
  bool is_3d() const;
  ///memory cont utilitys
  bool is_contiguous() const;   // same as v_is_contiguous()
  bool is_contiguous_0() const; // contiguous for dims >= 1
  bool is_contiguous_1() const; // contiguous for dims >= 2
  bool is_contiguous_2() const;
  bool is_contiguous_n(int n) const;
  bool is_contiguously_allocated() const; // returns whether the tensor elements are allocated as one contiguous block of memory (no gaps, but permutation ok)

  bool is_contiguous_channels(); // true for tensor that is stored in memory as CxWxHxN and has been permuted to WxHxCxN
  bool is_contiguous_rows();     // true if the elements in dimension 0 are contiguous, or there is just 1 block of elements
 ...
 */
V_API v_tensor* v_new_tensor(v_ctx* ctx, v_data_type type, int n_dims, const int64_t* ne);
//creates 1d tensor. call v_new_tensor()
V_API v_tensor* v_new_tensor_1d(v_ctx* ctx, v_data_type type, int64_t ne0);
//creates 2d tensor. call v_new_tensor()
V_API v_tensor* v_new_tensor_2d(v_ctx* ctx, v_data_type type, int64_t ne0, int64_t ne1);
//creates 3d tensor. call v_new_tensor()
V_API v_tensor* v_new_tensor_3d(v_ctx* ctx, v_data_type type, int64_t ne0, int64_t ne1, int64_t ne2);
//creates 4d tensor. call v_new_tensor()
V_API v_tensor* v_new_tensor_4d(v_ctx* ctx, v_data_type type, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3);
//creates a tensor. call new object to ctx
V_API v_tensor* v_new_tensor_impl(v_ctx* ctx, v_data_type type, int n_dims, const int64_t* ne, v_tensor* view_src, size_t view_offs);
//create tensor with same structure of source
V_API v_tensor* v_dup_tensor(v_ctx* ctx, const v_tensor* src);
 ...
 */
V_API v_tensor* v_view_impl(v_ctx* ctx, v_tensor* a, int n_dims, const int64_t* ne, size_t offset);
//Creates a view on an existing tensor. call v_view_impl()
V_API v_tensor* v_view_1d(v_ctx* ctx, v_tensor* a, int64_t ne0, size_t offset);
//Creates a view on an existing tensor. call v_view_impl()
V_API v_tensor* v_view_2d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, size_t nb1, size_t offset);
//Creates a view on an existing tensor. call v_view_impl()
V_API v_tensor* v_view_3d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2, size_t nb1, size_t nb2, size_t offset);
//Creates a view on an existing tensor. call v_view_impl()
V_API v_tensor* v_view_4d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3, size_t nb1, size_t nb2, size_t nb3, size_t offset);
//Creates a view on an existing tensor with same stride, same dims
V_API v_tensor* v_tensor_view(v_ctx* ctx, v_tensor* src);
/**
...
V_API v_tensor* v_reshape(v_ctx* ctx, v_tensor* a, v_tensor* b);
//Creates view tensor and return with reshaped 1d. call v_reshape()
V_API v_tensor* v_reshape_1d(v_ctx* ctx, v_tensor* a, int64_t ne0);
//Creates view tensor and return with reshaped 2d. call v_reshape()
V_API v_tensor* v_reshape_2d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1);
//Creates view tensor and return with reshaped 3d. call v_reshape()
V_API v_tensor* v_reshape_3d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2);
//Creates view tensor and return with reshaped 4d. call v_reshape()
V_API v_tensor* v_reshape_4d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3);
 ...
 */
V_API v_tensor* v_cont(v_ctx* ctx, v_tensor* a);
//util function return v_cont_4d with src
V_API v_tensor* v_cont_1d(v_ctx* ctx, v_tensor* a, int64_t ne0);
//util function return v_cont_4d with src
V_API v_tensor* v_cont_2d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1);
//util function return v_cont_4d with src
V_API v_tensor* v_cont_3d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2);
//operations s.t create new tensor with new shape, mem contigious
V_API v_tensor* v_cont_4d(v_ctx* ctx, v_tensor* a, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3);

/**
 * @brief creates a view on an existing tensor with new axis.
 * @param ctx       ptr to the computation context
 * @param a         src tensor
 * @param axis0       ne with new axis for source tensor
 * @param axis1       ne with new axis for source tensor
 * @param axis2       ne with new axis for source tensor
 * @param axis3       ne with new axis for source tensor
 * @return          ptr to the tensor specifies view
 */
V_API v_tensor* v_permute(v_ctx* ctx, v_tensor* a, int axis0, int axis1, int axis2, int axis3);
// alias for v_permute(ctx, a, 1, 0, 2, 3)
V_API v_tensor* v_transpose(v_ctx* ctx, v_tensor* a);
V_API v_tensor* v_get_rows(v_ctx* ctx,
                           v_tensor* a,  // data
