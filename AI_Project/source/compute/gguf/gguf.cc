...
struct gguf_kv {
    ...
    size_t get_ne() const {
        if (type == GGUF_TYPE_STRING) {
            ...
            MML_ASSERT(is_array || ne == 1);
            return ne;
        }
        const size_t type_size = gguf_type_size(type);
        MML_ASSERT(data.size() % type_size == 0);
    ...
    const T & get_val(const size_t i = 0) const {
        MML_ASSERT(type_to_gguf_type<T>::value == type);
        if constexpr (std::is_same<T, std::string>::value) {
            MML_ASSERT(data_string.size() >= i+1);
            return data_string[i];
        }
        const size_t type_size = gguf_type_size(type);
        MML_ASSERT(data.size() % type_size == 0);
        MML_ASSERT(data.size() >= (i+1)*type_size);
    ...
    void cast(const enum gguf_type new_type) {
        ...
        MML_ASSERT(data.size() % new_type_size == 0);
        type = new_type;
    }
};

struct gguf_tensor_info {
    struct MmlTensor t; // for holding the equivalent info
...
struct gguf_reader {
    ...
    bool read(enum v_type & dst) const {
        int32_t tmp = -1;
        if (!read(tmp)) {
            return false;
        }
...
bool gguf_read_emplace_helper(const struct gguf_reader & gr, std::vector<struct gguf_kv> & kv, const std::string & key, const bool is_array, const size_t n) {
    if (is_array) {
        ...
        try {
            ...
            v_LOG_ERROR("%s: encountered length_error while reading value for key '%s'\n", __func__, key.c_str());
            return false;
        } catch (std::bad_alloc &) {
            v_LOG_ERROR("%s: encountered bad_alloc error while reading value for key '%s'\n", __func__, key.c_str());
            return false;
        }
...
struct gguf_context * gguf_init_from_file_impl(FILE * file, struct gguf_init_params params) {
    ...
    // file magic
    {
        ...
        if (!ok) {
            v_LOG_ERROR("%s: failed to read magic\n", __func__);
            gguf_free(ctx);
            return nullptr;
        }
        ...
        for (uint32_t i = 0; i < magic.size(); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                ...
                v_LOG_ERROR("%s: invalid magic characters: '%c%c%c%c', expected 'GGUF'\n", __func__, c0, c1, c2, c3);
                gguf_free(ctx);
                return nullptr;
            }
        }
    ...
    if (ok && gr.read(ctx->version)) {
        if (ok && ctx->version == 0) {
            v_LOG_ERROR("%s: bad GGUF version: %" PRIu32 "\n", __func__, ctx->version);
            ok = false;
        }
        ...
        */
        if (ok && (ctx->version & 0x0000FFFF) == 0x00000000) {
            v_LOG_ERROR("%s: failed to load model: this GGUF file version %" PRIu32 " is extremely large, is there a mismatch between the host and model endianness?\n", __func__, ctx->version);
            ok = false;
        }

        if (ok && ctx->version == 1) {
            v_LOG_ERROR("%s: GGUFv1 is no longer supported, please use a more up-to-date version\n", __func__);
            ok = false;
        }
        if (ok && ctx->version > GGUF_VERSION) {
            v_LOG_ERROR("%s: this GGUF file is version %" PRIu32 " but this software only supports up to version %d\n",
    ...
    if (ok && gr.read(n_tensors)) {
        ...
        if (n_tensors < 0 || n_tensors > int64_t(SIZE_MAX/sizeof(gguf_tensor_info))) {
            v_LOG_ERROR("%s: number of tensors is %" PRIi64 " but must be in [0, %zu]\n",
    ...
    if (ok && gr.read(n_kv)) {
        ...
        if (n_kv < 0 || n_kv > int64_t(SIZE_MAX/sizeof(gguf_kv))) {
            v_LOG_ERROR("%s: number of key value pairs is %" PRIi64 " but must be in [0, %zu]\n",
    ...
    if (!ok) {
        v_LOG_ERROR("%s: failed to read header\n", __func__);
        gguf_free(ctx);
        return nullptr;
    }
    ...
    // KV pairs
    {
        for (int64_t i = 0; ok && i < n_kv; ++i) {
            ...
            try {
                ...
                v_LOG_ERROR("%s: encountered length_error while reading key %" PRIi64 "\n", __func__, i);
                ok = false;
            } catch (std::bad_alloc &) {
                v_LOG_ERROR("%s: encountered bad_alloc error while reading key %" PRIi64 "\n", __func__, i);
                ok = false;
            }
            for (size_t j = 0; ok && j < ctx->kv.size(); ++j) {
                if (key == ctx->kv[j].key) {
                    v_LOG_ERROR("%s: duplicate key '%s' for tensors %zu and %" PRIi64 " \n", __func__, key.c_str(), j, i);
                    ok = false;
                }
            }
            if (!ok) {
            ...
            switch (type) {
                ...
                default:
                    {
                        v_LOG_ERROR("%s: key '%s' has invalid GGUF type %d\n", __func__, key.c_str(), type);
                        ok = false;
                    } break;
            }
        }
        ...
        if (!ok) {
            v_LOG_ERROR("%s: failed to read key-value pairs\n", __func__);
            gguf_free(ctx);
            return nullptr;
        }
        MML_ASSERT(int64_t(ctx->kv.size()) == n_kv);

        const int alignment_idx = gguf_find_key(ctx, GGUF_KEY_GENERAL_ALIGNMENT);
        ctx->alignment = alignment_idx == -1 ? GGUF_DEFAULT_ALIGNMENT : gguf_get_val_u32(ctx, alignment_idx);

        if (ctx->alignment == 0 || (ctx->alignment & (ctx->alignment - 1)) != 0) {
            v_LOG_ERROR("%s: alignment %zu is not a power of 2\n", __func__, ctx->alignment);
            gguf_free(ctx);
            return nullptr;
        }
    }
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor name
        {
            ...
            try {
                ...
                v_LOG_ERROR("%s: encountered length_error while reading tensor name %" PRIi64 "\n", __func__, i);
                ok = false;
            } catch (std::bad_alloc &) {
                v_LOG_ERROR("%s: encountered bad_alloc error while reading tensor name %" PRIi64 "\n", __func__, i);
                ok = false;
            }
            if (name.length() >= v_MAX_NAME) {
                v_LOG_ERROR("%s: tensor name %" PRIi64 " is too long: %zu >= %d\n", __func__, i, name.length(), v_MAX_NAME);
                ok = false;
                break;
            }
            v_set_name(&info.t, name.c_str());

            // make sure there are no duplicate tensor names
            for (int64_t j = 0; ok && j < i; ++j) {
                if (strcmp(info.t.name, ctx->info[j].t.name) == 0) {
                    v_LOG_ERROR("%s: duplicate tensor name '%s' for tensors %" PRIi64 " and %" PRIi64 "\n", __func__, info.t.name, j, i);
                    ok = false;
                    break;
                }
            }
        ...
        // tensor shape
        {
            ...
            if (n_dims > MML_MAX_DIMS) {
                v_LOG_ERROR("%s: tensor '%s' has invalid number of dimensions: %" PRIu32 " > %" PRIu32 "\n",
                    __func__, info.t.name, n_dims, v_MAX_DIMS);
                ok = false;
                break;
            }
            for (uint32_t j = 0; ok && j < MML_MAX_DIMS; ++j) {
                info.t.ne[j] = 1;
                if (j < n_dims) {
                ...
                if (info.t.ne[j] < 0) {
                    v_LOG_ERROR("%s: tensor '%s' dimension %" PRIu32 " has invalid number of elements: %" PRIi64 " < 0\n",
            ...
            if (ok && ((INT64_MAX/info.t.ne[1] <= info.t.ne[0]) ||
                ...
                v_LOG_ERROR("%s: total number of elements in tensor '%s' with shape "
        ...
        // tensor type
        {
            ...
            if (info.t.type < 0 || info.t.type >= v_TYPE_COUNT) {
                v_LOG_ERROR("%s: tensor '%s' has invalid ggml type %d (%s)\n",
                    __func__, info.t.name, info.t.type, v_type_name(info.t.type));
                ok = false;
                break;
            }
            const size_t  type_size = v_type_size(info.t.type);
            const int64_t blck_size = v_blck_size(info.t.type);

            // check that row size is divisible by block size
            if (blck_size == 0 || info.t.ne[0] % blck_size != 0) {
                v_LOG_ERROR("%s: tensor '%s' of type %d (%s) has %" PRId64 " elements per row, "
                    "not a multiple of block size (%" PRId64 ")\n",
                    __func__, info.t.name, (int) info.t.type, v_type_name(info.t.type), info.t.ne[0], blck_size);
                ok = false;
                break;
            }
    ...
    if (!ok) {
        v_LOG_ERROR("%s: failed to read tensor info\n", __func__);
        gguf_free(ctx);
        return nullptr;
    }
    MML_ASSERT(int64_t(ctx->info.size()) == n_tensors);

    // we require the data section to be aligned, so take into account any padding
    if (fseek(file, MML_PAD(ftell(file), ctx->alignment), SEEK_SET) != 0) {
        v_LOG_ERROR("%s: failed to seek to beginning of data section\n", __func__);
        gguf_free(ctx);
        return nullptr;
    }
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (size_t i = 0; i < ctx->info.size(); ++i) {
            ...
            if (ti.offset != ctx->size) {
                v_LOG_ERROR("%s: tensor '%s' has offset %" PRIu64 ", expected %zu\n",
                    __func__, ti.t.name, ti.offset, ctx->size);
                v_LOG_ERROR("%s: failed to read tensor data\n", __func__);
                gguf_free(ctx);
                return nullptr;
            }
            size_t padded_size = MML_PAD(v_nbytes(&ti.t), ctx->alignment);
            if (SIZE_MAX - ctx->size < padded_size) {
                v_LOG_ERROR("%s: tensor '%s' size overflow, cannot accumulate size %zu + %zu\n",
    ...
    if (params.ctx != nullptr) {
        ...
        // otherwise, we load the binary blob into the created v_context as well, and point the "data" members of
        //   the v_tensor structs to the appropriate locations in the binary blob

        // compute the exact size needed for the new v_context
        const size_t mem_size =
            params.no_alloc ?
            (n_tensors    )*v_tensor_overhead() :
            (n_tensors + 1)*v_tensor_overhead() + ctx->size;

        struct v_init_params pdata = {
            /*mem_size   =*/ mem_size,
            /*mem_buffer =*/ nullptr,
            /*no_alloc   =*/ params.no_alloc,
        };

        *params.ctx = v_init(pdata);
        if (*params.ctx == nullptr) {
            v_LOG_ERROR("%s: failed to initialize ggml context for storing tensors\n", __func__);
            gguf_free(ctx);
            return nullptr;
        }

        struct v_context * ctx_data = *params.ctx;

        struct MmlTensor * data = nullptr;

        if (!params.no_alloc) {
            data = v_new_tensor_1d(ctx_data, v_TYPE_I8, ctx->size);

            ok = ok && data != nullptr;

            if (ok) {
                v_set_name(data, "GGUF tensor data binary blob");
            }
            ...
            if (!ok) {
                v_LOG_ERROR("%s: failed to read tensor data binary blob\n", __func__);
                v_free(ctx_data);
                *params.ctx = nullptr;
                gguf_free(ctx);
                return nullptr;
        ...
        v_set_no_alloc(ctx_data, true);

        // create the tensors
        for (size_t i = 0; i < ctx->info.size(); ++i) {
            const struct gguf_tensor_info & info = ctx->info[i];

            struct MmlTensor * cur = v_new_tensor(ctx_data, info.t.type, MML_MAX_DIMS, info.t.ne);
        ...
        if (!ok) {
            v_LOG_ERROR("%s: failed to create tensors\n", __func__);
            v_free(ctx_data);
            *params.ctx = nullptr;
            gguf_free(ctx);
            return nullptr;
        ...
        v_set_no_alloc(ctx_data, params.no_alloc);
    }
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    FILE * file = v_fopen(fname, "rb");

    if (!file) {
        v_LOG_ERROR("%s: failed to open GGUF file '%s'\n", __func__, fname);
        return nullptr;
    }
...
const char * gguf_get_key(const struct gguf_context * ctx, int64_t key_id) {
    MML_ASSERT(key_id >= 0 && key_id < gguf_get_n_kv(ctx));
...
enum gguf_type gguf_get_arr_type(const struct gguf_context * ctx, int64_t key_id) {
    MML_ASSERT(key_id >= 0 && key_id < gguf_get_n_kv(ctx));
    MML_ASSERT(ctx->kv[key_id].is_array);
    return ctx->kv[key_id].get_type();
}

const void * gguf_get_arr_data(const struct gguf_context * ctx, int64_t key_id) {
    MML_ASSERT(key_id >= 0 && key_id < gguf_get_n_kv(ctx));
    MML_ASSERT(ctx->kv[key_id].get_type() != GGUF_TYPE_STRING);
...
size_t gguf_get_tensor_offset(const struct gguf_context * ctx, int64_t tensor_id) {
    MML_ASSERT(tensor_id >= 0 && tensor_id < gguf_get_n_tensors(ctx));
...
enum v_type gguf_get_tensor_type(const struct gguf_context * ctx, int64_t tensor_id) {
    MML_ASSERT(tensor_id >= 0 && tensor_id < gguf_get_n_tensors(ctx));
...
size_t gguf_get_tensor_size(const struct gguf_context * ctx, int64_t tensor_id) {
    MML_ASSERT(tensor_id >= 0 && tensor_id < gguf_get_n_tensors(ctx));
    return v_nbytes(&ctx->info[tensor_id].t);
}
...
static void gguf_check_reserved_keys(const std::string & key, const T val) {
    if (key == GGUF_KEY_GENERAL_ALIGNMENT) {
        if constexpr (std::is_same<T, uint32_t>::value) {
            MML_ASSERT(val > 0 && (val & (val - 1)) == 0 && GGUF_KEY_GENERAL_ALIGNMENT " must be power of 2");
        } else {
            v_UNUSED(val);
            v_ABORT(GGUF_KEY_GENERAL_ALIGNMENT " must be type u32");
        }
    }
}
...
void gguf_add_tensor(
        ...
        const struct MmlTensor * tensor) {
    MML_ASSERT(tensor);
    if (gguf_find_tensor(ctx, tensor->name) != -1) {
        v_ABORT("duplicate tensor name: %s", tensor->name);
    }

    struct gguf_tensor_info ti;
    ti.t = *tensor;
    ti.offset = ctx->info.empty() ? 0 :
        ctx->info.back().offset + MML_PAD(v_nbytes(&ctx->info.back().t), ctx->alignment);
    ctx->info.push_back(ti);
}

void gguf_set_tensor_type(struct gguf_context * ctx, const char * name, enum v_type type) {
    const int64_t tensor_id = gguf_find_tensor(ctx, name);
    if (tensor_id < 0) {
        v_ABORT("tensor not found: %s", name);
    }
    struct MmlTensor * tensor = &ctx->info[tensor_id].t;
    const size_t  type_size = v_type_size(type);
    const int64_t blck_size = v_blck_size(type);

    tensor->type = type;
    MML_ASSERT(tensor->ne[0] % blck_size == 0 && "tensor row size not divisible by block size of new type");
    ...
    for (int64_t i = tensor_id + 1; i < n_tensors; ++i) {
        ctx->info[i].offset = ctx->info[i - 1].offset + MML_PAD(v_nbytes(&ctx->info[i - 1].t), ctx->alignment);
    }
}
...
struct gguf_writer_base {
    ...
    void write(const enum v_type & val) {
        write(int32_t(val));
    }
    ...
    void write_tensor_meta(const struct gguf_tensor_info & info) {
        ...
        const uint32_t n_dims = v_n_dims(&info.t);
        write(n_dims);
...
struct gguf_writer_buf final : public gguf_writer_base {
    ...
    void write_tensor_data(const struct gguf_tensor_info & info, const size_t offset_data, const size_t alignment) override {
        MML_ASSERT(buf.size() - offset_data == info.offset);

        MML_ASSERT(v_is_contiguous(&info.t));
        const size_t offset = buf.size();
        const size_t nbytes = v_nbytes(&info.t);

        buf.resize(offset + nbytes);
        if (info.t.buffer) {
            v_backend_tensor_get(&info.t, buf.data() + offset, 0, nbytes);
        } else {
            MML_ASSERT(info.t.data);
...
struct gguf_writer_file final : public gguf_writer_base {
    ...
    void write_tensor_data(const struct gguf_tensor_info & info, const size_t offset_data, const size_t alignment) override {
        MML_ASSERT(written_bytes - offset_data == info.offset);
        ...
        if (info.t.buffer) {
            v_backend_tensor_get(&info.t, buf.data(), 0, nbytes);
        } else {
            MML_ASSERT(info.t.data);
...
bool gguf_write_to_file(const struct gguf_context * ctx, const char * fname, bool only_meta) {
    FILE * file = v_fopen(fname, "wb");

    if (!file) {
        v_LOG_ERROR("%s: failed to open file '%s' for writing GGUF data\n", __func__, fname);
        return false;
    }
    ...
    try {
        ...
        v_LOG_ERROR("%s: failed to write GGUF data into '%s': %s\n", __func__, fname, ex.what());
        fclose(file);
        return false;
    }
