...
void vk_load_shaders(vk_device& device) {
  ...
  auto const& fa_spec_constants = [&](FaCodePath path, uint32_t hsk, uint32_t hsv, uint32_t clamp, v_data_type type,
    ...
    // The same D_split value is used_bits__ for both HSK and HSV, so just base it on the union of the LSBs.
  ...
  for (uint32_t s = 0; s < CONV_SHAPE_COUNT; ++s) {
    ...
    if (device->subgroup_shuffle &&
      ...
      conv2d_BS_CRS   = std::min(
        device->subgroup_size,
        conv2d_BS_CRS); // CRS block size should be capped at subgroup size for correctness when shuffle is used_bits__.
    }
