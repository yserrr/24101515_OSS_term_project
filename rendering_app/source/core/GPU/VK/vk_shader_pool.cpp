...
namespace gpu
{
  ...
  VkShaderPool::~VkShaderPool()
  {
    ...
    {
      vkDestroyShaderModule(ctx__->deviceh__,
                            shader,
                            nullptr);
    }
  }
  ...
  VkShaderModule VkShaderPool::compile(std::string filepath,
                                       shaderc_shader_kind flag)
  {
    ...
    VK_ASSERT(vkCreateShaderModule(ctx__->deviceh__, &ci, nullptr, &shader));
    return shader;
  }

  std::uintptr_t VkShaderPool::getShader(const std::string& filePath, shaderc_shader_kind flag)
  {
    VkShaderModule shaderBin = compile(filePath, flag);
    this->shaderBin = reinterpret_cast<std::uintptr_t>(shaderBin);
