...
#include "../render_graph_legacy.hpp"
#include <vulkan/vulkan.h>
#include <array>
#include <stdexcept>

RenderPassPool::RenderPassPool(RenderPassPoolCreateInfo &info) : device(info.device),
                                                                 simpleForwardPass_(VK_NULL_HANDLE),
                                                                 colorFormat(info.colorFormat) {};

RenderPassPool::~RenderPassPool()
{
  if (simpleForwardPass_ != VK_NULL_HANDLE)
  {
    vkDestroyRenderPass(device, simpleForwardPass_, nullptr);
  }
}
...
// RenderPass CreateInfo
  ...
  if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &simpleForwardPass_) != VK_SUCCESS)
  {
    throw std::runtime_error("failed to create render pass!");
  }
  return simpleForwardPass_;
}
