...
Mesh::Mesh(const std::vector<VertexAll> &vertices,
           ...
           MemoryAllocator &allocator) : vertices_(vertices), indices(indices), allocator(allocator)
{
  recenterMesh();
  reNomalCompute();
...
void Mesh::recenterMesh()
{
  ...
  centroid /= static_cast<float>(vertices_.size());
...
void Mesh::reNomalCompute()
{
  ...
  {
    VertexAll &v0 = vertices_[indices[i]];
    VertexAll &v1 = vertices_[indices[i + 1]];
    VertexAll &v2 = vertices_[indices[i + 2]];
